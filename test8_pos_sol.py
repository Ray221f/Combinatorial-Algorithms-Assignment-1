...
def calculate_rank(n, adj, precedence_pairs):
    """Calculate the rank in the Trotter-Johnson ordering of the valid topological sort"""
    ...
    if not valid_ordering:
        return "Impossible to satisfy the conditions"  # No valid permutation exists
    
    # Step 3: Calculate the rank of the valid permutation in Trotter-Johnson ordering
    # Convert the topological ordering to a permutation index in lexicographical order
...
def main():
    ...
    result = calculate_rank(n, adj, precedence_pairs)
    print(result)
